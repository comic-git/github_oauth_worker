...
The worker uses the normal web authorization flow after its popup handshake. Do not enable OAuth
during installation: GitHub makes the Setup URL unavailable when that setting is enabled. The setup
flow is required both after a new installation and after repositories are added or removed from an
...
2. Confirm GitHub redirects to `<WORKER_BASE_URL>/setup?installation_id=...`.
3. Complete the worker's fresh user authorization and repository confirmation.
4. Change the installation's repository selection, then confirm GitHub redirects to the Setup URL
   again because **Redirect on update** is enabled.
5. Confirm the worker rejects a manually altered `installation_id` unless the newly authorized user
   token proves access to that installation and selected repository.
